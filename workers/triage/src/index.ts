...
const handler = consumeQueue<TriageMessage>(
  async ({ env, sb, msg, attempt, beat }) => {
    ...
    if (env.STRUCTURE_QUEUE && allPages?.length) {
      // ⚡ Bolt: Optimize queue dispatch by chunking to avoid Cloudflare Workers' 100-message limit
      const promises = [];
      for (let i = 0; i < allPages.length; i += 100) {
        const chunk = allPages.slice(i, i + 100);
        promises.push(
          env.STRUCTURE_QUEUE.sendBatch(
            chunk.map((page: { id: string }) => ({ body: { run_id: runId, page_id: page.id } }))
          )
        );
      }
