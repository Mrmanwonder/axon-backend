...
5. Apply D1 migrations remotely, then `POST /v1/admin/capabilities/probe` with the admin bearer token and retain its complete JSON response as `capability-probe.json`. The endpoint persists the individual Gemini, Tavily, and document-vision results in D1, bound to the exact deployment SHA and config revision, and emits the exact certification artifact. Confirm that both the requested and served model are exactly `gemini-3.5-flash-lite`; any mismatch or false flag is a release blocker.
