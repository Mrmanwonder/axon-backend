...
Generate `capability-probe.json` by making an authenticated `POST` request to `/v1/admin/capabilities/probe` on the deployed intelligence Worker. Its response is already the `axon-capability-probe.v1` artifact expected by this verifier. It exercises Gemini structured output and thinking, Tavily search plus extraction against an authoritative source, and the private document-vision service's synthetic image path through both live readers. The synthetic page contains no student data. Preserve the response exactly; do not hand-edit passing flags. The nested `details` object is diagnostic and contains no prompt, source text, image bytes, or secret.
