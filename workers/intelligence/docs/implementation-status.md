...
- D1 provenance for deployments, routes, prompts, schemas, traces, evidence, claims, provider health, document stages, evaluation, shadow results, and capability probes. The authenticated release probe now exercises live Gemini structured output/thinking, Tavily search/extract authority, and both private document readers through a synthetic image before emitting the certification artifact. Gemini calls fail closed unless both the live Supabase route and the provider-reported served model are exactly `gemini-3.5-flash-lite`; readiness accepts only probes recorded for the current deployment SHA and config revision.
